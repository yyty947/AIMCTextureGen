...
- Phase 5 修订：2026-08-03，见
  [`2026-08-03-phase-5-four-candidate-generation-design.md`](2026-08-03-phase-5-four-candidate-generation-design.md)
...
首个 MVP 从用户主动导入的既有 Java 版资源包开始。应用读取 `pack.mcmeta`，选择对应的内置材质目录配置，按标准相对路径判断普通方块材质的覆盖状态。用户从未覆盖项中选择一个目标，选择 0–8 张包内或上传贴图作为风格参考，可选上传一张结构参考图，生成四个候选并在后处理后采用一个结果。采用结果只写入项目工作副本，最终导出为新的 ZIP；原始资源包保持不变。
...
6. 手动选择 0–8 张包内或上传风格参考图；存在参考图时通过 IP-Adapter 直接作为图像条件，没有参考图时运行 prompt-only workflow。
...
React + TypeScript + Vite WebUI
   ...
   +-----------+------------+----------------+------------------+
   |           |            |                |                  |
Project   JavaPackAdapter  TextureProcessor  GenerationService  GenerationCoordinator
Workspace      |                              |                  |
               v                              +--------+---------+
        Catalog Registry                              |
                                                      v
...
- `exports/` 只由用户主动导出创建。
- 风格参考可以通过工作副本内相对路径引用，也可以上传到项目级
  `uploads/style-references/`；结构参考上传到
  `uploads/structure-references/`。创建任务时，实际推理输入复制到任务自己的
  `inputs/` 并保存 SHA-256，后续工作副本或图库变化不改变旧任务。
...
- 一个目录条目目标；
- 16、32 或 64 的最终逻辑分辨率；
- 0–8 张工作副本内或项目上传的风格参考；
- 用户补充提示词；
- 可选结构参考；
- 固定模型配置；
- 创建时确定并持久化的原生执行批次及每批 base seed；
- 并行方式 1、2 或 4；
- 可折叠高级参数。

基础提示词由目标 `display_name`、`prompt_terms`、无缝方块像素贴片约束和用户补充描述按版本化纯函数组合。默认负面提示词排除物品图标、居中主体、白色留边、边框、透视、软边、渐变、文字和水印。MVP 不调用额外视觉模型自动把参考图转换成文字。

### 7.2 风格与结构

- 0 张风格参考时绕过 CLIP Vision/IP-Adapter，运行 prompt-only workflow。
- 1–8 张风格参考时通过 IP-Adapter 直接提供图像条件。
...
每个原生批次只有一个 base seed。批次内候选由该 seed 的批量噪声流和批次内
位置区分。任务创建时持久化完整批次计划；seed 只读，重试沿用，新任务重新
生成。应用不承诺候选序号在改变并行度后保持相同输出。
...
Phase 5 的 0–8 张风格参考、原生批次 seed、单活动任务协调器和 version 2
workflow 边界见
[`ADR-0003`](../../adr/0003-native-batch-seeds-and-generation-coordinator.md)。
已验证的 version 1 profile 不原地修改；version 2 必须重新完成真实 GPU
门禁后才能标记为 verified。
...
3. 参考与描述：选择 0–8 张包内或上传风格参考，上传可选结构参考，编辑补充描述。
4. 生成候选：选择分辨率、并行方式；折叠高级区包含负面提示词、条件性的 denoise/风格强度和只读模型配置。任务创建后只读展示批次 seed；MVP 不直接暴露采样器或 workflow 节点。
5. 采用与导出：逐张出现候选，展示最终贴图、最近邻预览、3×3 平铺、seam score 和参数；显式采用后才写入工作副本。

向导允许在任务创建前返回前一步修改配置。开始新任务不会删除旧候选。整个应用
同时只允许一个非终态生成任务；用户可以取消当前任务而不必等待全部候选完成。
取消运行只取消未完成部分，已生成候选继续保留。
...
queued 任务在重启后保持 queued，但不会自动启动 GPU；用户必须显式继续。
重启前仍在生成或后处理的任务转为 `JOB_INTERRUPTED`，保留已完成候选和
已验证原始图。重试保留原批次计划和 seed；如果某批原始图完整，只补确定性
后处理，否则使用原 seed 重跑整个原生批次。
