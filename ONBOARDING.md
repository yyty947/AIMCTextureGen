...
- 第三阶段 Task 6 已实现可丢弃的 SQLite schema-1 查询索引：`ProjectIndex` 每次操作使用有限 busy timeout 的独立连接、开启外键、显式事务写入，并只保存冻结的项目/任务摘要；UUID 文本、枚举和数值边界由数据库约束，时间统一为可正确排序的 UTC 文本。索引不保存 prompt、seed、文件系统路径、失败技术详情、source hash 或产物。`replace_snapshot` 在普通 `.aimctexturegen/` 目录内构建 `index.sqlite3.tmp`，完成完整性、外键和行数校验并关闭句柄后才原子替换；失败保留旧索引并清理临时库。`IndexService` 从权威 manifest/job JSON 模型重建，guarded upsert/query 在 SQLite 失败后只重建并重试一次，第二次失败返回稳定 `INDEX_UNAVAILABLE`。仍未接入 API、WebUI、ComfyUI 或真实模型。
...
当前工作分支为 `codex/phase-3-durable-jobs`。第三阶段 Task 1 至 Task 6 已完成；下一项是 Task 7“启动迁移与中断任务恢复”，负责恢复 active job、汇总恢复问题并从最终 JSON 状态重建索引。继续只执行 `docs/superpowers/plans/2026-07-27-phase-3-durable-jobs-and-recovery.md`，不接入真实模型、ComfyUI 或生产目录。

## 接手步骤

1. 运行 `git status --short`，确认并保留当前未提交改动。
2. 阅读 `AGENTS.md`、路线图、第二阶段计划和第三阶段可执行计划，了解已合并的 processing 契约以及即将建立的持久化接口。
3. 从 Task 7 开始继续逐项完成 RED → GREEN → commit；不要重复 Task 1–6 或提前执行后续任务。
...
2026-07-29 第三阶段 Task 6 的索引门禁 `.\.venv\Scripts\python -W error -m pytest backend\tests\index -v` 为 15 passed；完整后端回归 `.\.venv\Scripts\python -W error -m pytest backend\tests` 为 559 passed。覆盖 schema version/外键/规范 UUID、跨时区时间排序、retry lineage、冻结摘要与敏感字段排除；覆盖完整快照重建、删除后重建，以及填充、验证和发布失败时旧索引可用且临时库清理；覆盖 SQLite 首次失败后的一次重建/重试和第二次失败的稳定 `INDEX_UNAVAILABLE`。SQLite 只作为 JSON 权威数据的可重建查询索引，未读取或修改资源包文件，也未接入 API、WebUI、ComfyUI 或模型。
