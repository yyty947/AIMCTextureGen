...
- 第三阶段 Task 9 已实现 React WebUI 项目恢复与只读任务历史：启动时独立读取项目列表和无路径恢复报告，选择已有项目后通过项目详情、实时覆盖、任务列表与任务详情 API 恢复面板；新导入项目立即加入列表并成为当前选择。`ProjectList` 与 `JobHistory` 只负责渲染和选择事件，网络状态、请求竞态、错误保留与重试由 `App` 统一管理。前端运行时严格拒绝非规范 UUID、非安全整数、错误四 seed/候选数量、重复 seed、未知状态、无时区时间、负 revision、自指 retry lineage 及失败状态不一致。恢复问题不会隐藏有效项目，`JOB_INTERRUPTED` 显示可读说明；本任务没有加入目标、参考或生成控制。宽桌面使用两列，400–900 px 窄桌面堆叠；浏览器人工复验留给 Task 10 的用户步骤，本任务未运行 UI 密集自动化。
...
2026-07-29 第三阶段 Task 9 自动化门禁：`Push-Location frontend; ..\runtime\node-v24.18.0-win-x64\npm.cmd test; ..\runtime\node-v24.18.0-win-x64\npm.cmd run build; Pop-Location` 为 4 个测试文件、53 项前端测试通过且 Vite 生产构建成功；相关后端 API 回归 `.\.venv\Scripts\python -W error -m pytest backend\tests\api -v` 为 72 passed。组件测试覆盖恢复项目选择、只读任务字段/四候选状态计数、retry lineage、`JOB_INTERRUPTED` 与空历史；App 测试覆盖启动列表、选择后详情/覆盖/历史加载、新导入选中、恢复问题隔离和保留选择的重试。未运行浏览器或 UI 密集自动化，正常桌面与 400/600/900 px 人工复验仍是 Task 10 的用户验证项。
