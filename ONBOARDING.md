...
- 第三阶段 Task 9 已实现 React WebUI 项目恢复与只读任务历史：启动时独立读取项目列表和无路径恢复报告，选择已有项目后通过项目详情、实时覆盖、任务列表与任务详情 API 恢复面板；新导入项目立即加入列表并成为当前选择。`ProjectList` 与 `JobHistory` 只负责渲染和选择事件，网络状态、组件/操作/请求代次、当前选择、错误保留与重试由 `App` 统一管理；失败导入保留旧面板，选择项目、新导入或卸载会使旧导入/覆盖重试操作整体失效，旧 success/catch/finally 不能覆盖新选择或清除较新 busy。已在后端成功但被后续操作取代的导入仍只合并到项目列表，不会触发覆盖、选择自身或改变较新 busy/error；只有卸载后的响应完全丢弃。迟到的列表/面板/覆盖成功或失败同样不能覆盖新选择，卸载后不会继续多阶段请求或提交状态。列表顺序保留但可变任务状态统一取详情快照，摘要创建时间按后端索引来源与 request 对照，真正的跨响应冲突稳定显示为 `INVALID_API_RESPONSE`。前端运行时严格拒绝非规范 UUID、非安全整数、错误四 seed/候选数量、重复 seed、未知状态、无时区时间、负 revision、自指 retry lineage、失败状态不一致、不安全项目相对路径、非法 manifest 风格参考、错误 getProject 身份、非法项目名/prompt 以及违反后端冻结模型的 job/candidate 生命周期与时间范围。恢复问题不会隐藏有效项目，`JOB_INTERRUPTED` 显示可读说明；本任务没有加入目标、参考或生成控制。宽桌面使用两列，400–900 px 窄桌面堆叠，长连续项目名和任务标题可强制换行；浏览器人工复验留给 Task 10 的用户步骤，本任务未运行 UI 密集自动化。
...
当前工作分支为 `codex/phase-3-durable-jobs`。第三阶段 Task 1 至 Task 9 已完成；下一项是 Task 10“完整门禁、桌面人工步骤与交接”，由用户按明确步骤执行正常桌面和 400/600/900 px 窗口复验。继续只执行 `docs/superpowers/plans/2026-07-27-phase-3-durable-jobs-and-recovery.md`，不接入真实模型、ComfyUI 或生产目录。

## 接手步骤

1. 运行 `git status --short`，确认并保留当前未提交改动。
2. 阅读 `AGENTS.md`、路线图、第二阶段计划和第三阶段可执行计划，了解已合并的 processing 契约以及即将建立的持久化接口。
3. 从 Task 10 开始完成完整自动化门禁、向用户移交桌面人工步骤并记录真实结果；不要重复 Task 1–9 或提前进入 Phase 4 实现。
...
2026-07-29 第三阶段 Task 8 第一轮评审修复后的 API 门禁 `.\.venv\Scripts\python -W error -m pytest backend\tests\api -v` 为 72 passed；恢复与真实重启门禁仍为 3 passed；完整后端回归 `.\.venv\Scripts\python -W error -m pytest backend\tests` 为 589 passed。API 测试覆盖 JSON array 到 tuple 的显式传输映射、服务端四 seed/ID、规范 UUID、严格请求、确定性任务顺序、详情、取消/retry lineage、not-found、revision conflict、非法转换、不安全引用、损坏 JSON、索引不可用和 technical-details 安全标记；lifespan 测试证明注入恢复只执行一次、报告不含绝对路径、默认服务图完整，并在 Windows 上可于 lifespan 内重命名 SQLite 文件，证明没有长期连接占用。评审回归额外证明未运行 lifespan 时的磁盘项目服务复用 `AppServices` 已组合或注入的 repository，只省略 SQLite index，不按路径另建仓库或读取配置的真实磁盘根。原导入、multipart 与项目 API 回归保持通过；未接入 WebUI、ComfyUI、模型、候选采用、导出或生产目录。

2026-07-29 第三阶段 Task 9 第四轮评审修复后的自动化门禁：`Push-Location frontend; ..\runtime\node-v24.18.0-win-x64\npm.cmd test -- --run; ..\runtime\node-v24.18.0-win-x64\npm.cmd run build; Pop-Location` 为 4 个测试文件、96 项前端测试通过且 Vite 生产构建成功；相关后端 API 回归 `.\.venv\Scripts\python -W error -m pytest backend\tests\api -v` 为 72 passed。确定性回归现同时证明选择已有项目后重叠导入 B/C 的乱序完成、旧 import finally 不得清除较新 import busy、旧 import 不得打断较新项目 coverage retry，以及已持久化的 superseded import 必须保持列表可见但不选择、不读取覆盖、不改变 busy/error；前三轮全部组件、竞态、严格解析、卸载和长名称回归继续通过。未运行浏览器或 UI 密集自动化，正常桌面与 400/600/900 px 人工复验仍是 Task 10 的用户验证项。
